...
// Generates briefing AND sends email in one call
...
Generate a morning market briefing. Respond ONLY with valid JSON (no markdown), exactly this structure:
{
  ...
  "opportunities": {"headline":"text","picks":[{"ticker":"T1","name":"Co1","thesis":"2 sentences","signal":"bullish"},{"ticker":"T2","name":"Co2","thesis":"2 sentences","signal":"bullish"},{"ticker":"NGX:T3","name":"Nigerian Co","thesis":"2 sentences","signal":"bullish"}],"disclaimer":"For informational purposes only. Not financial advice."}
}`;
...
<tr><td style="text-align:center;font-family:monospace;font-size:10px;letter-spacing:3px;color:#7a7a8a;text-transform:uppercase;padding-bottom:6px">AI-POWERED FINANCIAL INTELLIGENCE</td></tr>
<tr><td style="text-align:center;font-size:44px;color:#0a0a0f;line-height:1.1">The Market <em style="color:#c9a84c">Brief</em></td></tr>
<tr><td style="text-align:center;font-family:monospace;font-size:11px;color:#7a7a8a;padding-top:8px">${dateStr.toUpperCase()}</td></tr>
<tr><td style="text-align:center;font-family:monospace;font-size:10px;color:#7a7a8a;border-top:1px solid #d4cfc4;border-bottom:1px solid #d4cfc4;padding:5px 0;margin-top:10px;letter-spacing:2px">S&amp;P 500 · NGX · GLOBAL · NEWS · OPPORTUNITIES</td></tr>
</table>
...
<div style="font-size:20px;color:#f5f0e8;font-style:italic">${data.summary?.headline || "Market Briefing"}</div>
...
<td width="50%" style="padding:14px 18px;border-right:1px solid #d4cfc4;vertical-align:top">
  <div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#7a7a8a;text-transform:uppercase;margin-bottom:5px">S&amp;P 500</div>
  <div style="font-size:30px;color:#0a0a0f">${data.sp500?.level || "—"}</div>
...
<td width="50%" style="padding:14px 18px;vertical-align:top">
  <div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#7a7a8a;text-transform:uppercase;margin-bottom:5px">NGX ALL-SHARE</div>
  <div style="font-size:30px;color:#0a0a0f">${data.nigeria?.allshare || "—"}</div>
...
<tr><td style="padding:14px 18px">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="border-bottom:1px solid #f0ece4">
  <tr><td style="padding:6px 0;font-family:monospace;font-size:12px">${i.name}</td>
  <td style="padding:6px 0;font-size:15px;text-align:center">${i.value}</td>
  <td style="padding:6px 0;font-family:monospace;font-size:12px;font-weight:700;text-align:right;color:${i.dir === "up" ? "#1a6b3a" : "#c0392b"}">${i.change}</td>
  </tr></table>`).join("")}
...
<tr><td style="padding:14px 18px">
  ...
  <div style="border-left:3px solid #c9a84c;padding:6px 0 6px 12px;margin-bottom:12px">
    <div style="font-size:13px;font-weight:700;color:#0a0a0f;margin-bottom:3px">${s.title}</div>
    <div style="font-size:12px;color:#7a7a8a;line-height:1.6">${s.impact}</div>
  </div>`).join("")}
</td></tr></table>
...
<tr><td style="padding:14px 18px">
  ...
  <div style="border-bottom:1px solid #e8e4dc;padding:10px 0">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      ...
      <td style="font-size:11px;color:#7a7a8a;text-align:center">${p.name}</td>
      <td style="text-align:right">${badge(p.signal)}</td>
    </tr></table>
    <div style="font-size:12px;color:#2c2c3a;line-height:1.7;margin-top:6px">${p.thesis}</div>
  </div>`).join("")}
  <div style="font-family:monospace;font-size:10px;color:#7a7a8a;margin-top:12px;line-height:1.6">${data.opportunities?.disclaimer || ""}</div>
</td></tr></table>
...
module.exports = async (req, res) => {
  ...
  const { email, provider, apiKey, smtpUser, smtpPass } = req.body;
  if (!email || !provider || !apiKey || !smtpUser || !smtpPass)
  ...
  try {
    ...
    res.json({ success: true, data, message: `Email sent to ${email}` });
  } catch (err) {
